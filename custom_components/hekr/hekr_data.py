...
from hekrapi import Device, DeviceID, DeviceResponseState, ACTION_COMMAND_RESPONSE, ACTION_DEVICE_MESSAGE, \
    LocalConnector, HekrAPIException
...
class HekrData:
    ...
    async def callback_update_entities(self, device: Device, message_id: int, state: DeviceResponseState,
                                       action: str, data: Tuple['Command', Dict, int]) -> NoReturn:
        """
        ...
        :return:
        """
        if device and action in (ACTION_COMMAND_RESPONSE, ACTION_DEVICE_MESSAGE) \
    ...
    async def create_device_registry_entry(self, device_id: DeviceID, config_entry_id: str) -> 'DeviceEntry':
    ...
    def collect_devices_for_entry(self, config_entry: config_entries.ConfigEntry) -> List[DeviceID]:
    ...
    async def update_account(self, account_id: Username) -> bool:
        ...
        for device_id, device in account.devices.items():
            ...
            if new_device_cfg is False:
                ...
                _LOGGER.warning('Device %s does not operate under supported protocol, and therefore will not be added.'
                                % device_id)
                continue
                ...
                _LOGGER.debug('Matched discovered device %s to supported protocol %s' % (device_id, protocol_id))
    ...
    def _refresh_updaters(self) -> NoReturn:
        ...
        for device_id, entities in self.device_entities.items():
            ...
            if update_commands:
                ...
                self.device_updaters[device_id] = (update_commands, self._create_updater(
                    device_id=device_id,
